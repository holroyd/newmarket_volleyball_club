...
sub fixtures {
	my $self = shift;
	my $sql = "select date, time, home_team, away_team, venue, league from league_fixtures_vw order by date_time";
	...
	while(my $leagueFixture = $sth->fetchrow_hashref()) {
		if(scalar(@fixturesByDate) == 0 || $fixturesByDate[$#fixturesByDate]->{"date"} ne $leagueFixture->{"date"}) {
			push(@fixturesByDate, {date => $leagueFixture->{"date"}, fixtures => []});
		}
